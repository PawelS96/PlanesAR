...
public class ArSession implements UpdateCallbackInterface {
    ...
    private final WeakReference<Control> mSessionControlRef;
    ...
    public ArSession(Control sessionControl) {
    ...
    private static class InitVuforiaTask extends AsyncTask<Void, Integer, Boolean> {
        ...
        private final WeakReference<ArSession> appSessionRef;

        InitVuforiaTask(ArSession session) {
            appSessionRef = new WeakReference<>(session);
        }

        private String key = "";

        protected Boolean doInBackground(Void... params) {
            ArSession session = appSessionRef.get();
    ...
    private static class ResumeVuforiaTask extends AsyncTask<Void, Void, Void> {
        ...
        ResumeVuforiaTask(ArSession session) {
            appSessionRef = new WeakReference<>(session);
        }
    ...
    private static class InitTrackerTask extends AsyncTask<Void, Integer, Boolean> {
        ...
        InitTrackerTask(ArSession session) {
            appSessionRef = new WeakReference<>(session);
        }
    ...
    private static class LoadTrackerTask extends AsyncTask<Void, Void, Boolean> {
        ...
        LoadTrackerTask(ArSession session) {
            appSessionRef = new WeakReference<>(session);
        }
    ...
    private static class StartVuforiaTask extends AsyncTask<Void, Void, Boolean> {
        ...
        StartVuforiaTask(ArSession session) {
            appSessionRef = new WeakReference<>(session);
        }
        ...
        protected void onPostExecute(Boolean result) {
            ...
            Control sessionControl = appSessionRef.get().mSessionControlRef.get();
